...
const defaultQuestions = [
  {
    ...
    options: [
      ...
      {
        id: 2,
        title: "IOS/Andrid App Development",
        subtitle: null,
...
export default function Estimate(props) {
  ...
  const handleSelected = (id) => {
    ...
    const previousSelected = currentActive[0].options.filter(option => option.selected)
    ...
    switch (newSelected.title) {
      case 'Custom Software Development':
        ...
        cleanState()
        break
      case 'IOS/Andrid App Development':
        setQuestions(softwareQuestions)
        setService(newSelected.title)
        cleanState()
        break
  ...
  const getTotal = () => {
    ...
    if (questions.length > 2) {
      ...
      cost = (cost - userCost.cost) * userCost.cost
    }
    setTotal(cost)
  ...
  const getCategory = () => {
    ...
    if (questions.length === 2) {
      const newCategory = questions.filter(question => question.title === "Which type of website are you wanting?")[0].options
      ...
      <Grid item container direction="column" lg style={{ marginRight: matchesMD ? 0 : "2em", marginBottom: "25em" }} alignItems="center">
              ...
              <Grid item container>
                {question.options.map((option, index) => (
                  <Grid
                    ...
                    key={`${option} ${index}`}
                    component={Button}
                    style={{ display: "grid", textTransform: "none", borderRadius: 0, backgroundColor: option.selected ? theme.palette.common.orange : null, marginBottom: matchesSM ? "1.5em" : 0 }}
        ...
        <Grid item>
          <Button variant="contained"
            ...
            onClick={() => { setDialogOpen(true); getTotal(); getPlatform(); getFeature(); getCutomFeature(); getCategory(); }}>
            Get Estimate
          </Button>
        </Grid>
      </Grid>
      ...
      >
        ...
        <DialogContent>
          <Grid container justify="space-around" direction={matchesSM ? "column" : "row"} alignItems={matchesSM ? "center" : undefined}>
            <Grid item container direction="column" md={7} style={{ maxWidth: matchesXS ? "100%" : "20em" }}>
            ...
            <Grid item container direction="column" md={5} style={{ maxWidth: matchesXS ? "100%" : "30em" }} alignItems={matchesSM ? "center" : undefined}>
              <Hidden smDown>
                <Grid item>
